...
import json, csv, re, datetime, MySQLdb, config
...
cursor = db.cursor()
cursor.execute("SELECT json FROM tweets_wse2013")
...
for tweet in data:
    ...
    if status.id in error_list_tweet_ids:
        ...
        if status.text.find('!') < 0:
            ...
            exclamation = 1
        if any(term in status.text for term in positive_terms):
            tweets_pos_term += 1
            pos_term = 1
        if any(term in status.text for term in negative_terms):
            tweets_neg_term += 1
            neg_term = 1
        if any(emoticon in status.text for emoticon in positive_emoticons):
            tweets_pos_emoticon += 1
            pos_emoticon = 1
        if any(emoticon in status.text for emoticon in negative_emoticons):
